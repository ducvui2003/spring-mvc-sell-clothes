...
public interface ProductCardDAO {
    ...
    int getQuantityProduct(@Bind(":visibility") boolean visibility);

    @SqlQuery("SELECT COUNT(*) FROM products WHERE visibility = :visibility AND id IN (<listId>)")
    int getQuantityProduct(@BindList("listId") List<Integer> listId, @Bind("visibility") boolean visibility);

    @SqlQuery("SELECT COUNT(*) FROM products WHERE id IN (<listId>)")
    int getQuantityProduct(@BindList("listId") List<Integer> listId);

    @SqlQuery("SELECT id, `name`, originalPrice, salePrice, visibility FROM products WHERE id IN (<listId>) LIMIT :limit OFFSET :offset")
    List<Product> pagingAndFilter(@Bind("listId") List<Integer> listId, @Bind("offset") int offset, @Bind("limit") int limit);
    ...
    List<Product> pagingAndFilter(@BindList("listId") List<Integer> listId, @Bind("offset") int pageNumber, @Bind("limit") int limit, @Bind("visibility") boolean visibility);
    ...
    List<Product> getProductByCategoryId(@Bind("categoryId") int categoryId);

    @SqlQuery("SELECT id FROM products WHERE name LIKE %:name%")
    List<Product> getIdProductByName(@Bind("name") String name);

    @SqlQuery("SELECT id FROM products WHERE createAt BETWEEN :dateBegin AND :dateEnd")
    List<Product> getProductByTimeCreated(@Bind("dateBegin") Date dateBegin, @Bind("dataEnd") Date dateEnd);
    ...
    @SqlQuery("""
            ...
            """)
    List<Category> getNameCategoryById(@Bind("id") int id);
    ...
    @SqlQuery("""
            ...
            WHERE products.id = :id
            """)
    List<Category> getCategoryByProductId(@Bind("id") int id);
    ...
    void updateVisibility(@Bind("id") int productId, @Bind("visibility") String visibility);
