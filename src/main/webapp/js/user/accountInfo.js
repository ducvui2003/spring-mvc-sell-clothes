...
$(document).ready(function () {
        ...
        const addressValidator = {
            ...
            submitHandler: function (form) {
                const formData = formDataToJson(form, {
                    provinceName: $("#inputProvince option:selected").text(),
                    districtName: $("#inputDistrict option:selected").text(),
                    wardName: $("#inputWard option:selected").text(),
                });
                alert(() => {
                    // Cập nhập địa chỉ
                    if (idSelected == null) {
                    ...
                    handleUpdateAddress(formData, idSelected);
                })
            }
        }

        function handleAddAddress(formData) {
            http({
        ...
        function handleUpdateAddress(formData, idSelected) {
            http({
                ...
                if (response.code == 200) {
                    ...
                    addressCustomer = addressCustomer.map(address => address.id === idSelected ? response.address : address);
                    loadDataToTable();
                } else {
                    Swal.fire({
                        title: "Lỗi!",
        ...
        function handleDeleteAddress(idSelected, element) {
            http({
                url: "/api/user/address/:id",
                type: 'DELETE',
                pathVariables: {
        ...
        function viewModal() {
            ...
            $(".btn__address-update").on("click", async function () {
                ...
                console.log("Clicked", idSelected)
                if (idSelected) {
                    const address = addressCustomer.find(address => {
                        return address.id === idSelected
                    });
            ...
            $(".btn__address-delete").on("click", function () {
                ...
                if (idSelected) {
                    alert(() => {
                            handleDeleteAddress(idSelected, element);
                        }, () => {
                            addressForm.resetForm();
                        }
                        , {
